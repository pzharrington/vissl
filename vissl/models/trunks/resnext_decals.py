...
from vissl.models.trunks.resnet_modified import ResNet
from vissl.models.trunks.resnet_modified import Bottleneck
...
@register_model_trunk("resnext_decals")
class ResNeXtDecals(nn.Module):
    """
    MODIFIED FOR DECALS DATA: Drop first maxpool in resnet, add option for stride = 1 or 2 in conv1
    ...
    def __init__(self, model_config: AttrDict, model_name: str):
        super(ResNeXtDecals, self).__init__()
        ...
        self.trunk_config = self.model_config.TRUNK.TRUNK_PARAMS.RESNETS_DECALS
        ...
        self.stride_flag = self.trunk_config.CONV1_STRIDE1
        logging.info(
            f"Building model: ResNeXtDecals"
            f"{self.depth}-{self.groups}x{self.width_per_group}d-"
            f"w{self.width_multiplier}-{self._norm_layer.__name__}"
            f"stride flag {self.stride_flag}"
        )
        ...
        conv1stride = 1 if self.stride_flag else 2
