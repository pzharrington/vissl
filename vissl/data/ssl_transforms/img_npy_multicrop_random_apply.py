...
@register_transform("ImgNpyMultiCropRandomApply")
class ImgPilMultiCropRandomApply(ClassyTransform):
    ...
    def __call__(self, image_list: List[ndarray]):
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> "ImgNpyMultiCropRandomApply":
        """
        Instantiates ImgNpyMultiCropRandomApply from configuration.

        Args:
            config (Dict): arguments for for the transform

        Returns:
            ImgNpyMultiCropRandomApply instance.
        """
        transforms = config.get("transforms", [])
        prob = config.get("prob", [])
        logging.info(f"ImgNpyMultiCropRandomApply | Using prob: {prob}")
