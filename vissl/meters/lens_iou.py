...
@register_meter("lens_iou")
class LensIoUMeter(ClassyMeter):
    """
    
    Compute IoU (jaccard similarity) of lens predicitons against truth:
                   IoU = TP/(FN+FP+TN)
    """
    ...
    def value(self):
        ...
        pred_probs = F.sigmoid(self._scores).detach().numpy()
        targets = self._targets.detach().numpy()

        top_preds = np.argsort(pred_probs)[:self.topN]
        true_lenses = np.nonzero(targets)[0]
        
        intersect = np.intersect1d(top_preds, true_lenses).shape[0]
        union = np.union1d(top_preds, true_lenses).shape[0]
        IoU = intersect/float(union)
        return {"IoU":IoU}
        '''
        # Thresholded prediciton
        preds = np.where(pred_probs >= self.thresh, 1, 0).astype(np.int64)
        cm = confusion_matrix(targets, preds)
        TP = cm[1,1]
        FP = cm[0,1]
        FN = cm[1,0]
        IoU = TP/(FP+TP+FN)
        return {"IoU":IoU, "Precision": TP/(TP+FP), "Recall": TP/(TP+FN)}
        '''
